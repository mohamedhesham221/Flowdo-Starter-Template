...
export default function LayoutFull({ children, title, item=false, tabs }) {
  ...
  return (
    ...
    <div className={`grid min-h-screen w-full ${sidePanel ? "md:grid-cols-[16.4rem_1fr] lg:grid-cols-[16.4rem_1fr]" : "md:grid-cols-[4.4rem_1fr] lg:grid-cols-[4.4rem_1fr]"} `}>
      ...
      <div className="flex flex-col">
        <Topbar title={title} item={item} tabs={tabs}/>
        
        <main className={` ${tabs ? 'mt-24' : 'mt-14' } font-sans flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-0 pt-2 sidebar-light bg-white`}>
          {children}
        </main>
      </div>
    </div>
